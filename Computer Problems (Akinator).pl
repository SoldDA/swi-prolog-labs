...
problem(sticking):-
    query('Is it possible to enter text with this device?').
...
% Добавлен объект 2 - без доп вопросов.
fault(the_keyboard):-
    problem(sticking).

% Добавлен объект 3 - с доп вопросами.
fault(fridge):-
    problem(large_size),
    problem(does_not_freeze).

% Добавлен объект 4 - с доп вопросами.
fault(frying_pan):-
    problem(oval_view),
    problem(its_burning_up).
