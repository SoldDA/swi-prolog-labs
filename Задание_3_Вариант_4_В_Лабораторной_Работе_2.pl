...
%  последовательность.
%  list_to_pairs(+List, -Pairs). Каждому элементу в списке присваивается
%  индекс.
...
% sort_pairs_by_value(+Pairs, -SortedPairs). Сортировка в порядке
% убывания
sort_pairs_by_value(Pairs, SortedPairs):-
    predsort(compare_pairs, Pairs, SortedPairs).

% Сравнение пар элементов(для сортировки). compare_pairs(-Znak, (_,
% +Value1), (_, +Value2)).
compare_pairs(Znak, (_, Value1), (_, Value2)):-
    compare(Znak, Value2, Value1), write(compare(Znak, Value2, Value1)).

% Извлекаем индексы из списка пар. extract_index([(+Index, _)|+T],
% [-Index|-Tail])
extract_index([], []).
...
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% 1.32 Дан целочисленный массив. Найти количество его локальных
% максимумов.
main_three(N, Max):-
    read_list(N, List), nl,
    write_list(List),
